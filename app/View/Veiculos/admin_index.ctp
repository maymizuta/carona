<div class="veiculos index">
	<h2><?php echo __('Veiculos'); ?></h2>
	<table cellpadding="0" cellspacing="0">
	<thead>
	<tr>
			<th><?php echo $this->Paginator->sort('id'); ?></th>
			<th><?php echo $this->Paginator->sort('placa'); ?></th>
			<th><?php echo $this->Paginator->sort('cor'); ?></th>
			<th><?php echo $this->Paginator->sort('cidade'); ?></th>
			<th><?php echo $this->Paginator->sort('estado'); ?></th>
			<th><?php echo $this->Paginator->sort('modeloDoCarro'); ?></th>
			<th><?php echo $this->Paginator->sort('marca'); ?></th>
			<th class="actions"><?php echo __('Actions'); ?></th>
	</tr>
	</thead>
	<tbody>
	<?php foreach ($veiculos as $veiculo): ?>
	<tr>
		<td><?php echo h($veiculo['Veiculo']['id']); ?>&nbsp;</td>
		<td><?php echo h($veiculo['Veiculo']['placa']); ?>&nbsp;</td>
		<td><?php echo h($veiculo['Veiculo']['cor']); ?>&nbsp;</td>
		<td><?php echo h($veiculo['Veiculo']['cidade']); ?>&nbsp;</td>
		<td><?php echo h($veiculo['Veiculo']['estado']); ?>&nbsp;</td>
		<td><?php echo h($veiculo['Veiculo']['modeloDoCarro']); ?>&nbsp;</td>
		<td><?php echo h($veiculo['Veiculo']['marca']); ?>&nbsp;</td>
		<td class="actions">
			<?php echo $this->Html->link(__('View'), array('action' => 'view', $veiculo['Veiculo']['id'])); ?>
			<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $veiculo['Veiculo']['id'])); ?>
			<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $veiculo['Veiculo']['id']), array('confirm' => __('Are you sure you want to delete # %s?', $veiculo['Veiculo']['id']))); ?>
		</td>
	</tr>
<?php endforeach; ?>
	</tbody>
	</table>
	<p>
	<?php
	echo $this->Paginator->counter(array(
		'format' => __('Page {:page} of {:pages}, showing {:current} records out of {:count} total, starting on record {:start}, ending on {:end}')
	));
	?>	</p>
	<div class="paging">
	<?php
		echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
		echo $this->Paginator->numbers(array('separator' => ''));
		echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
	?>
	</div>
</div>
<div class="actions">
	<h3><?php echo __('Actions'); ?></h3>
	<ul>
		<li><?php echo $this->Html->link(__('New Veiculo'), array('action' => 'add')); ?></li>
	</ul>
</div>
